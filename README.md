...
## Notes
- Embedding model defaults to `text-embedding-3-small` with `OPENAI_EMBEDDING_DIM=768`.
- Chat model defaults to `gpt-5-mini`.
...
- If chat/upload fail with provider errors, verify `OPENAI_API_KEY` and your OpenAI account's billing/credits.
